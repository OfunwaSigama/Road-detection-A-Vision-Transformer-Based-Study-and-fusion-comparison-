...
from src.models import RGBBaseline
...
def main():
    ...
    all_results = {}
    progress_file = Path("results/rgb_baseline_progress.json")
    if progress_file.exists():
        with open(progress_file, 'r') as f:
            all_results = json.load(f)
        print(f"Loaded {len(all_results)} completed folds: {list(all_results.keys())}")
    ...
    for i, test_seq in enumerate(sequences):
        ...
        print(f"\n{'='*80}\nFOLD {i+1}/8: Test={test_seq}, Val={val_seq}\n{'='*80}")

        train_ds = Kitti360RGBDataset(config['data']['root'], train_seqs, 'train', config['data']['image_size'])
        val_ds   = Kitti360RGBDataset(config['data']['root'], [val_seq], 'val', config['data']['image_size'])
        test_ds  = Kitti360RGBDataset(config['data']['root'], [test_seq], 'val', config['data']['image_size'])
    ...
    if all_results:
        ...
        with open("results/rgb_baseline_final.json", 'w') as f:
